...
public abstract class LanguageModel implements Serializable {
    ...
    public float[] getWordEntropies(String[] sent) {
        ...
        if (ukModels != null) {
            for (int i = 0; i < sent.length; i++)
                if (!isInVocab(sent[i])) {
                    unkEnts[i] = ukModels[ids[i + 1] - 2].getSentenceEntropy(TextUtils.splitChars(sent[i]));
    ...
    public float getSentenceEntropy(String[] sent) {
        return MathUtils.sum(getWordEntropies(sent));
    }

    public float getSentenceSimpleEntropy() {
        return MathUtils.sum(simpleEnts);
    }

    public float getSentenceUnknownEntropy() {
        return MathUtils.sum(unkEnts);
    }

    public float getSentenceClassEntropy() {
        return MathUtils.sum(classEnts);
    }
    ...
    private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
        ...
        try {
            vocab = (SymbolSet) in.readObject();
        } catch (NullPointerException e) {
        }
        vocabFrequency = in.readInt();
        vocabLimit = in.readInt();
        try {
            startSymbol = (String) in.readObject();
        } catch (NullPointerException e) {
        }
        try {
            terminalSymbol = (String) in.readObject();
        } catch (NullPointerException e) {
        }
        try {
            ukSymbol = (String) in.readObject();
        } catch (NullPointerException e) {
        }
        try {
            ukModels = (LanguageModel[]) in.readObject();
        } catch (NullPointerException e) {
        }
        if (ukModels != null) ukModelCount = ukModels.length;
        else ukModelCount = 1;
        try {
            classMap = (ClassMap) in.readObject();
        } catch (NullPointerException e) {
        }
    ...
    public void setUnknownSymbol(String ukSymbol) {
