...
public class GTSmoother extends NgramSmoother {
    ...
    public final static String ABV = "gt";
    ...
    public void process(NgramLM lm, NgramNode node, float[] discounted, int i, int lev) {
        ...
        //if(bo < -.0001f) {
        //	throw new IllegalArgumentException("Illegal backoff value " + bo + " in " + modelling.getNodeName(node));
        //else
        if (bo < 0.0f)
            bo = 0.0f;
        if (i == 0 && smoothUnigrams) {
            ...
            if (unk.getScore() != 0.0f)
                unk.setScore(MathUtils.logAddition(unk.getScore(), (float) Math.log10(bo)));
            else {
                lm.getNgramCounts()[0]++;
