package modelling.infrastructure.kylm.ngram;
...
public final class BranchNode extends NgramNode {
    ...
    protected List<NgramNode> children = null;
    ...
    public NgramNode getChild(int id, int add) {
        ...
        if (child == null && add != ADD_NONE) {
            ...
            if (parent == null)
                ...
                ((ArrayList) children).add(mid, child);
            childCount++;
        }
        return child;
    }
    ...
    @Override
    public void setChildren(List<NgramNode> newChildren) {
        if (parent == null) {
    ...
    private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
        ...
        if (size != 0) {
            ...
            children = new ArrayList<>(Math.abs(size));
            ((ArrayList) children).ensureCapacity(Math.abs(size));
