...
import modelling.infrastructure.kylm.ngram.reader.TextArraySentenceReader;
...
public class NgramLM extends LanguageModel implements Serializable {
	...
	protected Set<String> ukWords = null;
	...
	public void trainModel(Iterable<String[]> sl) throws Exception {
		...
		if(ukModels != null && ukWords != null) {
			...
			for(int i = 0; i < ukModelCount; i++)
				ukw.add(new LinkedList<>());
			for(String s : ukWords)
				ukw.get(findUnknownId(s)-2).add(TextUtils.join(" ", TextUtils.splitChars(s)));
